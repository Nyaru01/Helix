...
public static class ResultsExportService
{
        ...
        double minimumCoverage, IEnumerable<BlastHit> hits)
    {
        ...
        html.AppendLine($"<main class=\"wrap\"><section class=\"cards\"><div class=\"card\"><div class=\"metric\">{rows.Count}</div><div class=\"label\">Samples analysed</div></div><div class=\"card\"><div class=\"metric accent\">{present}</div><div class=\"label\">Present</div></div><div class=\"card\"><div class=\"metric\">{detectionRate:F0}%</div><div class=\"label\">Detection rate</div></div><div class=\"card\"><div class=\"metric\">{below}</div><div class=\"label\">Below thresholds</div></div></section><input id=\"filter\" placeholder=\"Filter samples or status…\" oninput=\"filterRows()\" style=\"width:100%;box-sizing:border-box;margin:0 0 16px;padding:12px;border-radius:9px;border:1px solid #293954;background:#101a2d;color:#eaf2ff\">");
        ...
        html.AppendLine("</tbody></table><p class=\"muted\">Coverage represents the best HSP coverage returned by BLAST. Click column headings to sort. This report is self-contained.</p></main><script>function filterRows(){let q=document.getElementById('filter').value.toLowerCase();document.querySelectorAll('tbody tr').forEach(r=>r.hidden=!r.innerText.toLowerCase().includes(q))}document.querySelectorAll('th').forEach((h,i)=>h.onclick=()=>{let b=h.closest('table').tBodies[0],r=[...b.rows];r.sort((a,z)=>a.cells[i].innerText.localeCompare(z.cells[i].innerText,undefined,{numeric:true}));r.forEach(x=>b.append(x))})</script></body></html>");
